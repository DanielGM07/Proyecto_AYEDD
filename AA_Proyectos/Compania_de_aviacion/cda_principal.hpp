...
string rCiudadToString(RCiudad x)
{
	...
	string sElecciones=to_string(x.elecciones);
	return sCiudad+sep+sElecciones;
}
...
RCiudad rCiudad(Ciudad ciudad,int elecciones)
{
	RCiudad a;
	a.ciudad = ciudad;
	a.elecciones = elecciones;
	return a;
}
...
bool rCiudadEquals(RCiudad a,RCiudad b)
{
	...
	if(a.elecciones!=b.elecciones) return false;
	return true;
}
...
string rVueloToString(RVuelo x)
{
	...
	string sPlazasRechazadas=to_string(x.plazasRechazadas);
	string sPlazasAceptadas=to_string(x.plazasAceptadas);
	return sVuelo+sep+sPlazasRechazadas+sep+sPlazasAceptadas;
}
...
RVuelo rVueloFromString(string s)
{
	...
	string t1 = getTokenAt(s,sep,1);
	x.plazasRechazadas=stoi(t1);
	string t2 = getTokenAt(s,sep,2);
	x.plazasAceptadas=stoi(t2);
	return x;
}
...
string rVueloToDebug(RVuelo x)
{
	...
	sout << ",";
	sout << x.plazasRechazadas;
	sout << ",";
	sout << x.plazasAceptadas;
	sout<< "]";
	return sout.str();
}
...
RVuelo rVuelo(Vuelo vuelo,int plazasRechazadas,int plazasAceptadas)
{
	RVuelo a;
	a.vuelo = vuelo;
	a.plazasRechazadas = plazasRechazadas;
	a.plazasAceptadas = plazasAceptadas;
	return a;
}
...
bool rVueloEquals(RVuelo a,RVuelo b)
{
	...
	if(a.plazasRechazadas!=b.plazasRechazadas) return false;
	if(a.plazasAceptadas!=b.plazasAceptadas) return false;
	return true;
}
...
string rClienteToString(RCliente x)
{
	...
	return sIdCliente+sep+sMillasAcum;
}
...
RCliente rCliente(int idCliente,int millasAcum)
{
	RCliente a;
